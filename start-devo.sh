...
docker rm -f crawler
docker run --privileged=true --name crawler --volume $(pwd):$JAR_FILE_FOLDER --env JAR_FILE_NAME=$JAR_FILE_NAME --env JAR_FILE_URL=$JAR_FILE_URL --env PARALLEL_TASKS_QNT=$PARALLEL_TASKS_QNT --env AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID --env AWS_SECRET_KEY=$AWS_SECRET_KEY $IMAGE_REPO_NAME:$IMAGE_TAG
